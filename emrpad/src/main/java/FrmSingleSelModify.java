...
public class FrmSingleSelModify extends Dialog {
    public AreaData m_Area =null;

    private String m_message;
    private Table m_table;

    public FrmSingleSelModify(Shell parent) {
        super(parent);
    }
    ...
    public void open() {
        ...
        if(null!= m_Area) {
            ...
            if (null != m_Area.EnumValue && m_Area.EnumValue.length > 0)
            {
                ...
                for (int i = 0; i < m_Area.EnumValue.length; i += 2)
                {
                    ...
                    if (m_Area.ValCode != null && m_Area.ValCode.equals(m_Area.EnumValue[i + 1]) == true)
                        itemSel=item;

                }

                if(null!=itemSel)
                    m_table.setSelection(itemSel);
        ...
        ok.addListener(SWT.Selection, event -> {
            ...
            int itemSelIndex=m_table.getSelectionIndex();
            if(itemSelIndex<0)
                itemSelIndex=0;
            ...
            if(itemCount>0)
            {
                ...
                for (int j=0;j<itemCount;j++)
                {
                    ...
                    if(j==itemSelIndex)
                    {
                        m_Area.Value= item.getText(0);
                        m_Area.ValCode = item.getText(1);
                    }
